...
| Black Hole | `src/scenes/blackhole/BlackHoleScene.ts` | Yes | Ray-marched Schwarzschild black hole with VR passthrough |
| N-Body | `src/scenes/nbody/NBodyScene.ts` | Yes | Gravitational simulator with presets |
...
### VR passthrough (Quest 3)

In VR, the black hole renders as a localized 2m sphere positioned in the user's real room via mixed reality passthrough. Two-tier architecture:

| Tier | Condition | Visual |
|------|-----------|--------|
| Tier 1 (fallback) | `camera-access` not granted | Dark void + Einstein ring glow + accretion disk over transparent passthrough |
| Tier 2 (full) | `camera-access` granted | Shader samples passthrough camera texture with gravitational lensing distortion |

Key implementation details:
- `XRManager` requests `passthrough` + `camera-access` as optional WebXR features
- `XRWebGLBinding.getCameraImage()` provides per-eye camera texture (when available)
- Fragment shader (`blackhole-vr.frag.glsl`) outputs alpha-blended fragments: absorbed rays → opaque black, escaped rays → transparent (Tier 1) or lensed camera feed (Tier 2)
- Early ray termination at `uSphereRadius * 1.1` instead of `MAX_DIST = 100.0` for Quest 3 performance
- Grab & drag: controller/hand ray intersects sphere → grip tracking moves `bhWorldPosition`
- VR panel: Mass cycle, Disk toggle, Reset Pos, Exit VR
...
| `XRManager.ts` | WebXR session management, passthrough + camera-access |
| `VRPanel.ts` | In-world VR UI panel |
...
| `shaders/blackhole-vr.vert.glsl` | VR stereo vertex shader (per-eye parallax) |
| `shaders/blackhole-vr.frag.glsl` | VR ray marching + passthrough lensing (two-tier) |
